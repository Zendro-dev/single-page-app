  ...
  */
  ...
  function getData() {
    ...
    if(lidsAssociated.current === undefined || lidsAssociated.current.associationName !== 'role') {
      ...
      */
      api.userB.getAssociatedRoles(graphqlServerUrl, item.id)
      .then(response => {
        if (
          response.data &&
          response.data.data
          ...
          */
          api.roleB.getCountItems(graphqlServerUrl, search, ops)
          .then(response => {
            if (
              response.data &&
              response.data.data
