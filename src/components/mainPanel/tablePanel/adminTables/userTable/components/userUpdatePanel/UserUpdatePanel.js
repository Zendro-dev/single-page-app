...
export default function UserUpdatePanel(props) {
  ...
  function getInitialValueOkStates() {
    ...
    initialValueOkStates.push({ key: 'email', valueOk: getAcceptableStatus('email', item.email) });
    initialValueOkStates.push({ key: 'password', valueOk: getAcceptableStatus('password', item.password) });
  ...
  function areThereChangedFields() {
    ...
    {
      if(values.current[keys[i]] !== item[keys[i]]) {
        return true;
      }
    }
    return false;
  ...
  function doSave(event) {
    ...
    //removeItems
    values.current.removeRoles = rolesIdsToRemove.current;

    /*
      API Request: updateItem
    */
    api.userB.updateItem(graphqlServerUrl, item.id, values.current)
      .then(response => {
        if (
          response.data &&
          response.data.data
  ...
  const handleUntransferFromRemove =(associationKey, itemId) => {
    if(associationKey === 'role') {
      for(var i=0; i<rolesIdsToRemove.current.length; ++i)
      {
        ...
        <Grid container justify='center'>
          <Grid item xs={12}>
            ...
            <UserAssociationsPage
              ...
              rolesIdsToRemove={rolesIdsToRemove.current}
