...
import NoAssocEnhancedTableHead from './components/No_assocEnhancedTableHead'
import NoAssocEnhancedTableToolbar from './components/No_assocEnhancedTableToolbar'
import NoAssocUploadFileDialog from './components/No_assocUploadFileDialog'
import NoAssocCursorPagination from './components/No_assocCursorPagination'
...
//lazy loading
const NoAssocCreatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Create-NoAssoc" */ './components/no_assoc-create-panel/No_assocCreatePanel')));
const NoAssocUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Update-NoAssoc" */ './components/no_assoc-update-panel/No_assocUpdatePanel')));
const NoAssocDetailPanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-NoAssoc" */ './components/no_assoc-detail-panel/No_assocDetailPanel')));
const NoAssocDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Delete-NoAssoc" */ './components/No_assocDeleteConfirmationDialog')));
//Plotly
const NoAssocPlotly = lazy(() => retry(() => import(/* webpackChunkName: "Plotly-NoAssoc" */ '../../../../plots/No_assocPlotly')));
...
export default function NoAssocEnhancedTable(props) {
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.no_assoc.getCountItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.no_assoc.getCountItems(graphqlServerUrl, search));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'no_assoc', method: 'getCount()', request: 'api.no_assoc.getCountItems'}];
            newError.path=['No_assocs'];
      ...
      //error
      .catch((err) => { //error: on api.no_assoc.getCountItems
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.no_assoc.getItems
    */
    let api = await loadApi("no_assoc");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.no_assoc.getItems(
      graphqlServerUrl,
      search,
      orderBy,
      order,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'no_assoc', method: 'getData()', request: 'api.no_assoc.getItems'}];
            newError.path=['No_assocs'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.no_assoc.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['no_assoc']
    &&(Object.keys(changes.lastModelChanged['no_assoc']).length===1)
    &&(changes.lastModelChanged['no_assoc'][Object.keys(changes.lastModelChanged['no_assoc'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.no_assoc.tableTemplate
    */
    let api = await loadApi("no_assoc");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.no_assoc.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'no_assoc', method: 'getCsvTemplate()', request: 'api.no_assoc.tableTemplate'}];
            newError.path=['No_assocs', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateNo_assoc.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "no_assoc-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.no_assoc.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='NoAssocEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='NoAssocEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <NoAssocEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='NoAssocEnhancedTable-tableBody'>
                            {
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'NoAssocEnhancedTable-row-iconButton-detail-'+item.idField}
                                          color="default"
                                          onClick={event => {
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'NoAssocEnhancedTable-row-iconButton-delete-'+item.idField}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...
NoAssocEnhancedTable.propTypes = {
