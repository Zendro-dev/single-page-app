  ...
  return (
    <Grid container justify='flex-start' alignItems='center' spacing={0}>
        ...
        <MuiPickersUtilsProvider libInstance={moment} utils={MomentUtils} locale={i18n.language}>
          <FormControl className={classes.formControl}>
            <KeyboardTimePicker
              ...
              InputAdornmentProps={{
                id: 'TimeField-User-input-inputAdornment-'+name,
              }}
              KeyboardButtonProps={{
                id: 'TimeField-User-input-inputAdornment-button-'+name,
              }}
              onChange={(date, value) => {
      ...
      <Grid item>
        {(valueOk!==undefined&&valueOk===1) 
          ...
          <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
            <Typography id={'TimeField-User-exists-'+name} variant="caption" color="primary">
              &#8707;
            </Typography>
          </Tooltip>
          : 
          <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
            <Typography id={'TimeField-User-notExists-'+name} variant="caption" color="textSecondary">
              &#8708;
            </Typography>
          </Tooltip>
        }
      ...
      {(valueAjv !== undefined && valueAjv.errors.length > 0) && (
        <Grid item id={'TimeField-User-ajvError-'+name} xs={12}>
