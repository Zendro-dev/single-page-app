  ...
  return (
    <Grid container justify='flex-start' alignItems='center' spacing={0}>
        ...
        <MuiPickersUtilsProvider libInstance={moment} utils={MomentUtils} locale={i18n.language}>
          <FormControl className={classes.formControl}>
            <KeyboardDatePicker
              ...
              InputAdornmentProps={{
                id: 'DateField-Role-input-inputAdornment-'+name,
              }}
              KeyboardButtonProps={{
                id: 'DateField-Role-input-inputAdornment-button-'+name,
              }}
              onChange={(date, value) => {
      ...
      <Grid item>
        {(valueOk!==undefined&&valueOk===1) 
          ...
          <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
            <Typography id={'DateField-Role-exists-'+name} variant="caption" color="primary">
              &#8707;
            </Typography>
          </Tooltip>
          : 
          <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
            <Typography id={'DateField-Role-notExists-'+name} variant="caption" color="textSecondary">
              &#8708;
            </Typography>
          </Tooltip>
        }
      ...
      {(valueAjv !== undefined && valueAjv.errors.length > 0) && (
        <Grid item id={'DateField-Role-ajvError-'+name} xs={12}> 
