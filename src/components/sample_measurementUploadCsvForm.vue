<template>
  <div class="col-xs-5 content">
    <ul v-for="record in errors" v-if="errors" class="list-group">
      <li class="list-group-item">
        <div class="alert alert-danger">
          <h4>Errors for sample_measurement {{record.record}}</h4> 
          <ul>
    ...
    <h4>Upload sample_measurements</h4>
      <form id="sample_measurement-form" enctype="multipart/form-data" novalidate v-on:submit.prevent="onSubmit">
  ...
  methods: {
    onSubmit() {
      ...
      if (t.$refs.uploadTable.value.indexOf('.xlsx') > 0) {
        var url = this.$baseUrl() + '/sample_measurements/upload_xlsx'
        var formElm = "xlsx_file"
      } else {
        var url = this.$baseUrl() + '/sample_measurements/upload_csv'
