...
`;
    ...
    //query
    let query = `query usersConnection($order: [orderUserInput], $search: searchUserInput, $pagination: paginationCursorInput!) {
...
`;
    ...
    let query = `query readOneUser($id:ID!, $search: searchRoleInput, $pagination: paginationCursorInput!) {
             readOneUser(id:$id) {
    ...
    if (check === 'ok') {
      //check type
      if (
        !response.data.data['readOneUser'] ||
        typeof response.data.data['readOneUser'] !== 'object' ||
        !response.data.data['readOneUser']['rolesConnection'] ||
        typeof response.data.data['readOneUser']['rolesConnection'] !==
          'object' ||
        !Array.isArray(
          response.data.data['readOneUser']['rolesConnection'].edges
        ) ||
        typeof response.data.data['readOneUser']['rolesConnection'].pageInfo !==
          'object' ||
  ...
  async getRolesCount(url, itemId, searchText, ops) {
    ...
    if (s) variables.search = s.search;
    //query
    let query = `query readOneUser($id:ID!, $search: searchRoleInput) {
             readOneUser(id:$id) {
    ...
    if (check === 'ok') {
      ...
      if (
        ...
        typeof response.data.data['readOneUser'] !== 'object' ||
        !Number.isInteger(
          response.data.data['readOneUser']['countFilteredRoles']
        )
      )
        return {
          data: response.data.data,
  ...
  async getNotAssociatedRolesCount(url, itemId, searchText, ops) {
    ...
    if (globals.REQUEST_LOGGER)
      logRequest('getNotAssociatedRolesCount.query1', query, variables);
    //request
    let response = await requestGraphql({ url, query, variables });
    let countA = null;
    //check
    ...
    if (s) variables.search = s.search;
    //query
    query = `query readOneUser($id:ID!, $search: searchRoleInput) {
             readOneUser(id:$id) {
              countFilteredRoles(search: $search)
       }}`;
     ...
     */
    if (globals.REQUEST_LOGGER)
      logRequest('getRolesCount.query2', query, variables);
    //request
...
`;
    ...
    let batchQuery = `query rolesConnection($id: String, $search: searchRoleInput, $pagination: paginationCursorInput!) {
               rolesConnection(search: $search, pagination: $pagination) {
                  pageInfo {startCursor endCursor hasPreviousPage hasNextPage}
                  edges {node {
                    ${qattributes}
    ...
    while (nonaEdges.length < requiredItems) {
      ...
      if (globals.REQUEST_LOGGER)
        logRequest(
          `getNotAssociatedRoles.batch.${iteration}`,
          batchQuery,
          batchVariables
        );
      ...
      for (
        ...
        if (
          batchConnection.edges[i].node['usersConnection'].edges.length === 0
        ) {
  ...
  async getBarchartData(url, attribute) {
    ...
    //query
    let batchQuery = `query usersConnection($pagination: paginationCursorInput!) {
