 ...
 */
  ...
  async tableTemplate(url) {
    ...
    let headers = null;
    //check
    let check = checkResponse(response, graphqlErrors, "csvTableTemplateRole_to_user");
    if(check === 'ok') {
      //check type
 ...
 */
  ...
  async getCountItems(url, searchText, ops) {
    ...
    //query
    let query =
      `query countRole_to_users($search: searchRole_to_userInput) { 
...
`;
    ...
    //query
    let query =
      `query role_to_usersConnection($order: [orderRole_to_userInput], $search: searchRole_to_userInput, $pagination: paginationCursorInput!) { 
    ...
    let items = null;
    //check
    let check = checkResponse(response, graphqlErrors, "role_to_usersConnection");
    if(check === 'ok') {
      //check type
      ...
      || typeof response.data.data["role_to_usersConnection"].pageInfo !== 'object' 
...
`;
    ...
    if(check === 'ok') {
        //check type
        if(!response.data.data["addRole_to_user"]
...
`;
    ...
    if(check === 'ok') {
        //check type
        if(!response.data.data["updateRole_to_user"]
