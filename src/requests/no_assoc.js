...
const config = {
  ...
  async getCountItems(url, searchText, ops) {
    ...
    //query
    let query = `query countNo_assocs($search: searchNo_assocInput) {
...
`;
    ...
    //query
    let query = `query no_assocsConnection($order: [orderNo_assocInput], $search: searchNo_assocInput, $pagination: paginationCursorInput!) {
...
`;
    ...
    //query
    let query = `mutation addNo_assoc(
          ${qvariables}
          ) { addNo_assoc(
          ${qparameters}
          ) {
    ...
    if (check === 'ok') {
      //check type
      if (
        !response.data.data['addNo_assoc'] ||
...
`;
    ...
    //query
    let query = `mutation updateNo_assoc(
          ${qvariables}
          ) { updateNo_assoc(
          ${qparameters}
          ) {
    ...
    if (check === 'ok') {
      //check type
      if (
        !response.data.data['updateNo_assoc'] ||
  ...
  async getBarchartData(url, attribute) {
    ...
    //query
    let batchQuery = `query no_assocsConnection($pagination: paginationCursorInput!) {
