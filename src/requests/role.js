...
export default {
    ...
    var query = 
      `mutation{ 
        updateRole(
          ...
          description: "${values.description}",
          addUsers: [${values.addUsers.join()}],
          removeUsers: [${values.removeUsers.join()}],    
          )
          { 
            id,
            name,
   ...
   * getUsersFilter
   * 
   * Get users filter records associated to the given role record
   * from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  getUsersFilter(url, itemId, label, sublabel, searchText, paginationOffset, paginationLimit, ops) {
    //search
   ...
   * getAssociatedUsers
   * 
   * Get the user-ids associated to the given role record 
   * from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item id.
   */
  getAssociatedUsers(url, itemId) {
    var query = 
      `{ readOneRole( id: ${itemId} ){ 
        usersFilter{ id } } }`;
