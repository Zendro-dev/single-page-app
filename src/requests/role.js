...
const config = {
  ...
  async getCountItems(url, searchText, ops) {
    ...
    //query
    let query = `query countRoles($search: searchRoleInput) {
...
`;
    ...
    //query
    let query = `query rolesConnection($order: [orderRoleInput], $search: searchRoleInput, $pagination: paginationCursorInput!) {
    ...
    if (check === 'ok') {
      //check type
      if (
        !response.data.data['rolesConnection'] ||
...
`;
    ...
    let query = `query readOneRole($id:ID!, $search: searchUserInput, $pagination: paginationCursorInput!) {
             readOneRole(id:$id) {
    ...
    if (check === 'ok') {
      //check type
      if (
        !response.data.data['readOneRole'] ||
        typeof response.data.data['readOneRole'] !== 'object' ||
        !response.data.data['readOneRole']['usersConnection'] ||
        typeof response.data.data['readOneRole']['usersConnection'] !==
          'object' ||
        !Array.isArray(
          response.data.data['readOneRole']['usersConnection'].edges
        ) ||
        typeof response.data.data['readOneRole']['usersConnection'].pageInfo !==
          'object' ||
  ...
  async getUsersCount(url, itemId, searchText, ops) {
    ...
    if (s) variables.search = s.search;
    //query
    let query = `query readOneRole($id:ID!, $search: searchUserInput) {
             readOneRole(id:$id) {
    ...
    if (check === 'ok') {
      ...
      if (
        ...
        typeof response.data.data['readOneRole'] !== 'object' ||
        !Number.isInteger(
          response.data.data['readOneRole']['countFilteredUsers']
        )
      )
        return {
          data: response.data.data,
  ...
  async getNotAssociatedUsersCount(url, itemId, searchText, ops) {
    ...
    //set query
    let query = `query countUsers($search: searchUserInput) {
    ...
    if (globals.REQUEST_LOGGER)
      logRequest('getNotAssociatedUsersCount.query1', query, variables);
    //request
    let response = await requestGraphql({ url, query, variables });
    let countA = null;
    //check
    ...
    if (s) variables.search = s.search;
    //query
    query = `query readOneRole($id:ID!, $search: searchUserInput) {
             readOneRole(id:$id) {
              countFilteredUsers(search: $search)
       }}`;
     ...
     */
    if (globals.REQUEST_LOGGER)
      logRequest('getUsersCount.query2', query, variables);
    //request
...
`;
    ...
    let batchPagination = {
      ...
      first: isForward ? batchSize : null,
      last: !isForward ? batchSize : null,
    };
    //search
    ...
    let batchQuery = `query usersConnection($id: String, $search: searchUserInput, $pagination: paginationCursorInput!) {
               usersConnection(search: $search, pagination: $pagination) {
                  pageInfo {startCursor endCursor hasPreviousPage hasNextPage}
                  edges {node {
                    ${qattributes}
    ...
    while (nonaEdges.length < requiredItems) {
      ...
      if (globals.REQUEST_LOGGER)
        logRequest(
          `getNotAssociatedUsers.batch.${iteration}`,
          batchQuery,
          batchVariables
        );
      ...
      if (check === 'ok') {
        //check type
        if (
          !response.data.data['usersConnection'] ||
      ...
      for (
        let i = 0;
        i < batchConnection.edges.length && nonaEdges.length < requiredItems;
        i++
      ) {
        //check: not associated item
        if (
          batchConnection.edges[i].node['rolesConnection'].edges.length === 0
        ) {
      ...
      let thereAreMoreNonaItems =
        batchEdges.length > requiredItems - nonaEdges.length;

      if (batchEdges.length > 0) {
        //if there are new non-associated items...
        //adjust pageInfo (start)
        if (nonaEdges.length === 0) {
          if (isForward) {
            nonaPageInfo.startCursor = batchEdges[0].cursor;
            nonaPageInfo.hasPreviousPage =
              batchConnection.pageInfo.hasPreviousPage;
          } else {
        ...
        if (isForward)
          nonaEdges = [
            ...nonaEdges,
            ...batchEdges.slice(0, requiredItems - nonaEdges.length),
          ];
        else
          nonaEdges = [
            ...batchEdges.slice(-(requiredItems - nonaEdges.length)),
            ...nonaEdges,
          ];
      }
       ...
       *
       */
      let thereAreMoreItems = isForward
        ? batchConnection.pageInfo.hasNextPage
        : batchConnection.pageInfo.hasPreviousPage;

      if (nonaEdges.length < requiredItems && thereAreMoreItems) {
        //adjust pagination for next batch associated ids
        batchPagination.after = isForward
          ? batchConnection.pageInfo.endCursor
          : null;
        batchPagination.before = !isForward
          ? batchConnection.pageInfo.startCursor
          : null;
        ...
        if (nonaEdges.length > 0) {
          if (isForward) {
            ...
            nonaPageInfo.hasNextPage =
              batchConnection.pageInfo.hasNextPage || thereAreMoreNonaItems;
          } else {
            nonaPageInfo.startCursor = nonaEdges[0].cursor;
            nonaPageInfo.hasPreviousPage =
              batchConnection.pageInfo.hasPreviousPage || thereAreMoreNonaItems;
          }
        }
        ...
        //return value
        return {
          value: { pageInfo: nonaPageInfo, edges: nonaEdges },
          message: 'ok',
          graphqlErrors: graphqlErrors.length > 0 ? graphqlErrors : undefined,
        };
      }
  ...
  async getBarchartData(url, attribute) {
    ...
    //query
    let batchQuery = `query rolesConnection($pagination: paginationCursorInput!) {
