...
export default {
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countAminoacidsequences(${s}) }`;
    }
    else {
      query = `{ countAminoacidsequences }`;
    }
  ...
  deleteItem(url, itemId) {
    var query = 
    `mutation{ 
      deleteAminoacidsequence(id: ${itemId}) }`;
   ...
   * 
   * Get transcript_counts filter records associated to the given aminoacidsequence record
   * from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  getTranscript_countsFilter(url, itemId, label, sublabel, searchText, paginationOffset, paginationLimit, ops) {
    ...
    if (s !== null) {
      query = 
      `{ readOneAminoacidsequence(id: ${itemId}) { 
   ...
   * 
   * Get the transcript_count-ids associated to the given aminoacidsequence record 
   * from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item id.
