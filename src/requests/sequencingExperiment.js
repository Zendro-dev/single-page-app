...
export default {
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('sequencingExperiment', searchText, ops);
    
    var query = '';

    //if has search
    if (s !== null) {
      query = `{ countSequencingExperiments(${s}) }`;
    }
    else {
      query = `{ countSequencingExperiments }`;
    }
  ...
  deleteItem(url, itemId) {
    var query = 
    `mutation{ 
      deleteSequencingExperiment(id: ${itemId}) }`;
