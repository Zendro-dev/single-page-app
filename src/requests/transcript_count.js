...
export default {
    ...
    var query = 
      `mutation{ 
        addTranscript_count(
          ...
          count: "${values.count}",
          tissue_or_condition: "${values.tissue_or_condition}",
          individual_id: "${values.individual_id}",
          aminoacidsequence_id: "${values.aminoacidsequence_id}",
          addAminoacidsequences: [${values.addAminoacidsequences.join()}],
          addIndividuals: [${values.addIndividuals.join()}],
          ) 
          { 
            id,
            gene,
    ...
    var query = 
      `mutation{ 
        updateTranscript_count(
          ...
          aminoacidsequence_id: "${values.aminoacidsequence_id}",
          addAminoacidsequences: [${values.addAminoacidsequences.join()}],
          removeAminoacidsequences: [${values.removeAminoacidsequences.join()}],    
          addIndividuals: [${values.addIndividuals.join()}],
          removeIndividuals: [${values.removeIndividuals.join()}],    
          )
          { 
            id,
            gene,
  ...
  deleteItem(url, itemId) {
    var query = 
    `mutation{ 
      deleteTranscript_count(id: ${itemId}) }`;
  ...
  /**
   * getAminoacidsequencesFilter
   * 
   * Get aminoacidsequences filter records associated to the given transcript_count record
   * from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  getAminoacidsequencesFilter(url, itemId, label, sublabel, searchText, paginationOffset, paginationLimit, ops) {
    //search
    var s = getSearchArgument('aminoacidsequence', searchText, ops);
    ...
    if (s !== null) {
      ...
      `{ readOneTranscript_count(id: ${itemId}) { 
        aminoacidsequencesFilter( ${s}, ${p} ) { 
          id,
          ${label}, 
          ${sublabel},
        }, 
        countFilteredAminoacidsequences( ${s} ) 
      } }`;
    }
    else {
      query = 
      `{ readOneTranscript_count(id: ${itemId}) { 
        aminoacidsequencesFilter( ${p} ) { 
          id, 
          ${label}, 
          ${sublabel},
        }, 
        countFilteredAminoacidsequences 
      } }`;
    }
   ...
   * 
   *  
   * getAssociatedAminoacidsequences
   * 
   * Get the aminoacidsequence-ids associated to the given transcript_count record 
   * from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item id.
   */
  getAssociatedAminoacidsequences(url, itemId) {
    var query = 
      `{ readOneTranscript_count( id: ${itemId} ){ 
        aminoacidsequencesFilter{ id } } }`;
  ...
  /**
   * getIndividualsFilter
   * 
   * Get individuals filter records associated to the given transcript_count record
   * from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  getIndividualsFilter(url, itemId, label, sublabel, searchText, paginationOffset, paginationLimit, ops) {
    //search
   ...
   * 
   * Get the individual-ids associated to the given transcript_count record 
   * from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item id.
   */
  getAssociatedIndividuals(url, itemId) {
    var query = 
      `{ readOneTranscript_count( id: ${itemId} ){ 
        individualsFilter{ id } } }`;
