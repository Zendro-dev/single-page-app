...
export default {
  ...
  getAssociationFilter(url, itemId, association, label, sublabel, searchText, paginationOffset, paginationLimit, ops) {
    var associationCp = association.charAt(0).toUpperCase() + association.slice(1);
    ...
    //if has search
    if (s !== null) {
      query = `{ readOneRole(id: ${itemId}) { ${association}Filter( ${s}, ${p} ) { id, ${label}, ${sublabel} }, countFiltered${associationCp}( ${s} ) } }`;
    }
    else {
      query = `{ readOneRole(id: ${itemId}) { ${association}Filter( ${p} ) { id, ${label}, ${sublabel} }, countFiltered${associationCp} } }`;
    }
  ...
  getAssociatedIds(url, itemId, association) {
    /*
      Get graphQL @query
    */
    var query = `{ readOneRole( id: ${itemId} ){ ${association}Filter{ id } } }`;
    ...
    var query = 
        `mutation{ 
          updateRole(
            ...
            description: "${values.description}", 
            addUsers: ${values.addUsers},
            removeUsers: ${values.removeUsers},
            )
            { 
              id,
              name,
  ...
  deleteItem(url, itemId) {
    /*
      Get graphQL @query
    */
    var query = `mutation{ deleteRole(id: ${itemId}) }`;
