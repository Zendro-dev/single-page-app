    ...
    dispatch(authRequest(values.user, values.password))
      ...
      .catch((err) => {
        ...
        if (err && err.response && err.response.status === 500) {
          ...
          newError.message = t(
            'modelPanels.errors.request.e1',
            'Error in request made to server.'
          );
          newError.locations = [
            {
              method: 'doLoginRequest()',
              action: 'authRequest',
              request: loginServerUrl,
            },
          ];
          ...
          newError.extensions = {
            error: {
              message: err.message,
              config: err.config ? { url: err.config.url } : undefined,
        ...
        case 'user':
          ...
          if (isUserValid(values.user)) {
            ...
            setErrorUser({
              ...
              text: t(
                'login.errors.e1',
                'Please enter a valid e-mail. Example: myEmail@company.abc'
              ),
            });
          }
          break;
