...
export function checkResponse(response, graphqlErrors, queryName) {
  ...
  if (!Array.isArray(graphqlErrors))
    throw new Error('internal_error: array expected');
  if (typeof queryName !== 'string')
    throw new Error('internal_error: string expected');

  //check: server error
  if (!response || !response.data) throw new Error('bad_server_response');
  //check: graphql errors
  if (response.data.errors)
    graphqlErrors.push({ query: queryName, errors: response.data.errors });
  //check: data errors
...
export function logRequest(name, query, variables) {
  ...
  console.group(
    '%cquery',
    'color: #03A9F4; font-weight: bold; padding-left: 4px'
  );
  console.log(`%c${query}`, 'color: grey; font-weight: normal;');
  console.groupEnd();
  console.log(
    '%cvariables  %o',
    'color: #4CAF50; font-weight: bold; padding-left: 4px',
    variables
  );
  if (variables) {
    console.groupCollapsed(
      '%cvariables',
      'color: grey; font-weight: bold; padding-left: 12px'
    );
    console.log(
      `%c${JSON.stringify(variables, null, 2)}`,
    ...
    if (variables.pagination) {
      console.groupCollapsed(
        '%cpagination',
        'color: grey; font-weight: normal; padding-left: 16px'
      );
      console.log(
        `%c${JSON.stringify(variables.pagination, null, 2)}`,
        'color: grey; font-weight: normal;'
      );
      console.groupEnd();
    }
    if (variables.search) {
      ...
      console.log(
        `%c${JSON.stringify(variables.search, null, 2)}`,
        'color: grey; font-weight: normal;'
      );
      console.groupEnd();
    }
    if (variables.order) {
      ...
      console.log(
        `%c${JSON.stringify(variables.order, null, 2)}`,
        'color: grey; font-weight: normal;'
      );
      console.groupEnd();
    }
 ...
 * Search utils
 */
export function getSearchArgument(
  filterName,
  searchText,
  ops,
  format,
  ...
  if (!getAttributes && !attributes)
    throw new Error('internal_error: attributes or getAttributes expected');
  if (getAttributes && !filterName)
    throw new Error('internal_error: filterName expected');
  if (searchText && typeof searchText !== 'string')
    throw new Error('internal_error: string expected in searchText');
          ...
          */
          if (o.type === 'Int' || o.type === 'Float' || o.type === 'String') {
            ...
            for (let k = 0; k < vkeys.length; k++) {
              ...
              onlyOrs += `{field:${
                vkeys[k]
              }, valueType: Array, value:"${va.join()}", operator:in},`;
              o_onlyOrs.push({
                field: vkeys[k],
                valueType: 'Array',
                value: va.join(),
          ...
          */
          if (o.type === 'Int' || o.type === 'Float' || o.type === 'String') {
            ...
            for (let k = 0; k < vkeys.length; k++) {
              ...
              ands += `{field:${
                vkeys[k]
              }, valueType: Array, value:"${va.join()}", operator:notIn},`;
              o_ands.push({
                field: vkeys[k],
                valueType: 'Array',
                value: va.join(),
...
function getIsoDateTime(text) {
   ...
   */
  if (
    /^\d{4}[/-][01]\d[/-][0-3]\d[T ][0-2]\d:[0-5]\d:[0-5]\d\.\d+/.test(text)
  ) {
    let M = text.slice(5, 7);
    let D = text.slice(8, 10);
...
export async function retry(
  p,
  retries = globals.MAX_DYNAMIC_IMPORT_RETRIES !== undefined
    ? globals.MAX_DYNAMIC_IMPORT_RETRIES
    : 10,
  interval = 200
) {
