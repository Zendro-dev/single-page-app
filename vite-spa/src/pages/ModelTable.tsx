// Ported from pages/[group]/[model]/index.tsx. Next-specific plumbing
// swapped:
//   - getStaticPaths/getStaticProps -> gone (no SSG in a CSR app); group/
//     model come straight from useParams() instead of pre-computed props.
//   - useRouter() -> useRouteQuery() (src/hooks/useRouteQuery.ts)
//   - useSession() (next-auth, bare call, unused result) -> removed
// Everything else - the actual table/import/export logic - is unmodified.
...
import { csvProcessing, jsonProcessing, bulkDownload } from 'zendro-bulk-create';
...
export default function ModelTable() {
  const { group = 'models', model: modelName = '' } = useParams();
  ...
  const handleExportCsv = async (): Promise<void> => {
    try {
      const name = modelName.slice(0, 1).toLowerCase() + modelName.slice(1);
      ...
      if (associations) {
        for (const [assocName, assocObj] of Object.entries<Record<string, string>>(
          associations
        )) {
          const addAssocName =
  ...
  const handleImportFile = async (
    ...
    try {
      ...
      if (file_extension === 'csv') {
        ...
        await csvProcessing(file, data_model_definition, true, globals, zendro.request);
        console.log('finish validation!');
        await csvProcessing(file, data_model_definition, false, globals, zendro.request);
        ...
        let sheet_name = globals.SHEET_NAME ? globals.SHEET_NAME : work_book.SheetNames[0];
        sheet_name = globals.SHEET_NAME == '' ? work_book.SheetNames[0] : globals.SHEET_NAME;

        const work_sheet = work_book.Sheets[sheet_name];
        const records = XLSX.utils.sheet_to_json(work_sheet, { raw: false });

        await jsonProcessing(records, data_model_definition, true, globals, zendro.request);
        console.log('finish validation!');
        await jsonProcessing(records, data_model_definition, false, globals, zendro.request);
        console.log('finish uploading!');
        showSnackbar(t('success.csv-import'), 'success');
      ...
      if (
        ...
        Object.keys(error as object)[0].includes('record')
      ) {
        showErrorTable(data_model_definition, error as Record<string, Record<string, any>>);
      }
    }
  };
            ...
            ? info.errors.extensions.input
              ...
              : JSON.parse(
                  info.subquery
                    ...
                    .split(')')[0]
                    .split(',')
                    .filter((pair: string) => pair.split(':')[0] === internalID)[0]
                    .split(':')[1]
                )
            : JSON.parse(
                info.subquery
  ...
  const handleOnCreate = (): void => {
    router.push(`/${group}/${modelName}/new`);
  };

  const handleOnRead = (primaryKey: string | number): void => {
    router.push(`/${group}/${modelName}/details?id=${primaryKey}`);
  };

  const handleOnUpdate = (primaryKey: string | number): void => {
    router.push(`/${group}/${modelName}/edit?id=${primaryKey}`);
  };
  ...
  return (
    <ModelBouncer object={modelName} action="read">
      <TableContainer className={classes.tableContainer}>
        ...
        <Table caption={t('model-table.caption', { modelName })} data-cy="record-table">
          <TableHeader
            actionsColSpan={
        ...
        <TablePagination
          ...
          hasFirstPage={
            model.apiPrivileges.backwardPagination ? pageInfo.hasPreviousPage : true
          }
          hasLastPage={
            model.apiPrivileges.backwardPagination ? pageInfo.hasNextPage : undefined
          }
          hasPreviousPage={
            model.apiPrivileges.backwardPagination ? pageInfo.hasPreviousPage : undefined
          }
