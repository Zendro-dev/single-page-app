...
export async function parseStaticModel(
  ...
  // Model definitions found in the wild aren't always consistently cased
  // (e.g. "SQL" vs "sql") - getModelApiPrivileges below switches on the
  // lowercase form, so an uppercase value would silently fall through to
  // its default case rather than actually matching. This was never caught
  // before since next-plugin-preval's build step never ran a real
  // typechecker against its own generated output.
  dataModel.storageType = dataModel.storageType.toLowerCase() as typeof dataModel.storageType;
