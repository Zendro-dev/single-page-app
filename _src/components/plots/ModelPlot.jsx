...
export default function NoAssocPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.no_assoc.getBarchartData
    */
    ...
    await api.no_assoc.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'no_assoc', method: 'getBarchartData()', request: 'api.no_assoc.getBarchartData'}];
            newError.path=['No_assocs', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.no_assoc.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
